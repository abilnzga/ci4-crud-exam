...
class ApplicationModel extends Model
{
    ...
    public function getUser($username = false, $userID = false)
    {
        if ($username) {
            return $this->db->table('users')
                ->select('*, users.id AS userID, user_role.id AS role_id, COALESCE(users.name, users.fullname) AS display_name, COALESCE(users.email, users.username) AS login_email')
    ...
    public function createUser($dataUser)
    {
        return $this->db->table('users')->insert([
            ...
            'password'    => password_hash($dataUser['inputPassword'], PASSWORD_BCRYPT),
